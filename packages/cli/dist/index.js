...
var _linesToClear, _isDiscardingStdin, _lineCount, _frameIndex, _lastSpinnerFrameTime, _options, _spinner, _stream, _id, _initialInterval, _isEnabled, _isSilent, _indent, _text, _prefixText, _suffixText, _Ora_instances, getFullPrefixText_fn, getFullSuffixText_fn, updateLineCount_fn;
var Ora = class {
  constructor(options) {
...
var registry_default = {
  Form: {
    files: ["Form.tsx"],
    npmDependencies: ["react-hook-form"],
  ...
  FormTextInput: {
    files: ["FormTextInput.tsx", "components/TextInput.tsx"],
    npmDependencies: ["react-hook-form", "@tabler/icons-react"],
    registryDependencies: ["Form", "Generic"]
  },
  FormSelectBox: {
    files: ["FormSelectBox.tsx", "components/SelectBox.tsx", "components/CheckBox.tsx"],
    npmDependencies: ["react-hook-form", "@tabler/icons-react"],
    registryDependencies: ["Form", "Generic"]
  },
  FormCheckBox: {
    files: ["FormCheckBox.tsx", "components/CheckBox.tsx"],
    npmDependencies: ["react-hook-form"],
    registryDependencies: ["Form", "Generic"]
  },
  FormTextArea: {
    files: ["FormTextArea.tsx", "components/TextArea.tsx"],
    npmDependencies: ["react-hook-form", "@tabler/icons-react"],
    registryDependencies: ["Form", "Generic"]
  },
  FormToggle: {
    files: ["FormToogle.tsx", "components/Toggle.tsx"],
    npmDependencies: ["react-hook-form", "@tabler/icons-react"],
    registryDependencies: ["Form", "Generic"]
  },
  FormNumeric: {
    files: ["FormNumeric.tsx", "components/Numeric.tsx"],
    npmDependencies: ["react-hook-form", "@tabler/icons-react", "react-use"],
    registryDependencies: ["Form", "Generic"]
  },
  FormDateTime: {
    files: ["FormDateTime.tsx", "components/DateTime.tsx"],
    npmDependencies: ["react-hook-form", "@tabler/icons-react"],
    registryDependencies: ["Form", "Generic"]
  },
  FormTime: {
    files: ["FormTime.tsx", "components/Time.tsx"],
    npmDependencies: ["react-hook-form", "@tabler/icons-react"],
    registryDependencies: ["Form", "Generic"]
  },
  FormPhoneInput: {
    files: ["FormPhoneInput.tsx", "components/PhoneInput.tsx"],
    npmDependencies: ["react-hook-form", "@tabler/icons-react", "react-phone-input-2"],
    registryDependencies: ["Form", "Generic"]
  },
  Button: {
    files: ["components/Button.tsx"],
    npmDependencies: ["@tabler/icons-react"],
    registryDependencies: []
  },
  RadioButton: {
    files: ["components/RadioButton.tsx"],
    npmDependencies: [],
    registryDependencies: []
  },
  TextEditor: {
    files: ["components/TextEditor.tsx"],
    npmDependencies: ["react-draft-wysiwyg", "draft-js", "draftjs-to-html", "html-to-draftjs"],
    registryDependencies: []
  },
  DatePicketBig: {
  ...
  ListBox: {
    ...
    npmDependencies: ["@tabler/icons-react", "react-router"],
    registryDependencies: []
  },
  SelectBoxGroup: {
...
async function installComponent(name, destPath, installed, npmDepsToInstall) {
  ...
  if (!component) {
    console.log(source_default.red(`  \u2717 Component "${name}" not found in registry`));
    return;
  }
  ...
  for (const file of component.files) {
    const targetFile = import_path.default.join(destPath, file);
    await import_fs_extra.default.ensureDir(import_path.default.dirname(targetFile));
    if (await import_fs_extra.default.pathExists(targetFile)) {
      console.log(source_default.gray(`  ~ ${file} (already exists, skipped)`));
      continue;
    }
    const content = await fetchFile(file);
    await import_fs_extra.default.writeFile(targetFile, content, "utf-8");
...
function detectPackageManager() {
  try {
    if (import_fs_extra.default.existsSync("bun.lockb")) return "bun";
    if (import_fs_extra.default.existsSync("pnpm-lock.yaml")) return "pnpm";
    if (import_fs_extra.default.existsSync("yarn.lock")) return "yarn";
  } catch {
  }
  return "npm";
}
function getConfigPath() {
  const configFile = import_path.default.resolve(process.cwd(), "makdos.config.json");
  if (import_fs_extra.default.existsSync(configFile)) {
    try {
      const config = JSON.parse(import_fs_extra.default.readFileSync(configFile, "utf-8"));
...
async function add(components, options) {
  if (components.length === 0) {
    ...
    console.log(source_default.gray("Usage: npx @makdosdev/form-ui add <component>"));
    console.log(source_default.gray("List:  npx @makdosdev/form-ui list\n"));
    return;
  }
  const destPath = import_path.default.resolve(process.cwd(), options.path ?? getConfigPath());
  ...
  try {
    for (const name of components) {
      if (!typedRegistry[name]) {
        ...
        console.log(source_default.gray(`Run "npx @makdosdev/form-ui list" to see available components.
`));
        return;
      }
    }
...
${name}:`));
    ...
    if (npmDepsToInstall.size > 0) {
      ...
      const installCmd = pm === "yarn" ? `yarn add ${depsList}` : pm === "pnpm" ? `pnpm add ${depsList}` : pm === "bun" ? `bun add ${depsList}` : `npm install ${depsList}`;
      console.log(source_default.bold(`
Installing dependencies:`));
      console.log(source_default.gray(`  ${installCmd}
`));
      const depSpinner = ora("Installing...").start();
      (0, import_child_process.execSync)(installCmd, { stdio: "ignore" });
      depSpinner.succeed("Dependencies installed");
    }
    console.log(source_default.green(source_default.bold("\nDone!\n")));
  } catch (err) {
...
function list() {
  ...
  const components = Object.keys(registry_default);
  components.forEach((name) => {
    const component = registry_default[name];
    const deps = component.registryDependencies.length > 0 ? source_default.gray(` (requires: ${component.registryDependencies.join(", ")})`) : "";
    console.log(`  ${source_default.cyan(name)}${deps}`);
  });
...
async function init() {
  ...
  try {
    ...
    const configPath = import_path2.default.resolve(process.cwd(), "makdos.config.json");
...
program2.name("makdos-form-ui").description("Add Makdos Form UI components to your project").version("0.1.0");
program2.command("init").description("Add the theme CSS file to your project").action(init);
program2.command("add [components...]").description("Add components to your project").option("-p, --path <path>", "destination path for components (overrides makdos.config.json)").action(add);
program2.command("list").description("List all available components").action(list);
program2.parse();
